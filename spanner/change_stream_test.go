...
const (
	envTestProjectID        = "SPANNER_TEST_PROJECT_ID"
	envTestInstanceID       = "SPANNER_TEST_INSTANCE_ID"
	envTestDatabaseID       = "SPANNER_TEST_DATABASE_ID"
	envTestCredentialBase64 = "SPANNER_TEST_CREDENTIAL_BASE64"
...
func initialize() {
	if os.Getenv(envTestProjectID) == "" || os.Getenv(envTestInstanceID) == "" || os.Getenv(envTestDatabaseID) == "" {
		fmt.Println(os.Getenv(envTestProjectID), os.Getenv(envTestInstanceID))
...
func TestReader(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.desc, func(t *testing.T) {
			dbPath := fmt.Sprintf("projects/%s/instances/%s/databases/%s", testProjectID, testInstanceID, testDatabaseID)
			reader, err := NewChangeStreamReaderWithConfig(ctx, dbPath, setupResult.streamID, ChangeStreamReaderConfig{
			...
			go reader.Read(readerContext, func(_ context.Context, result *ReadResult) error {
